...
export interface MockFollowup {
  ...
  last: string
  due: string
  state: 'due' | 'scheduled' | 'overdue' | 'sent'
}
...
export interface MockBusiness {
  ...
  plan: 'Pro' | 'Standard' | 'Basic' | 'Enterprise'
  leads: number
  added: string
  color: string
  icon: string
...
export interface MockChartPoint { label: string; v: number }
...
export const LEADS: MockLead[] = [
  ...
  },
  {
    ...
    reply: "Hi Michelle,\n\nApologies we missed your same-day request. We'd love to help next time.\n\nBest regards,\nAcme Home Services",
    timeline: [
...
export const CONVERSATIONS: MockConversation[] = [
  {
    ...
    thread: [
      { kind: 'customer', text: "Hi! I'm looking to get my roof inspected. Can you let me know availability this week?", time: '10:24 AM' },
      { kind: 'auto', text: "Thanks for reaching out! We've received your request and will be in touch within 24 hours.", time: '10:24 AM' },
      { kind: 'owner', text: "Hi Jessica, we have availability this week — would you prefer morning or afternoon?", time: '10:31 AM' },
      { kind: 'customer', text: 'Morning would be great, thanks!', time: '10:42 AM' },
      { kind: 'ai-draft', text: "Perfect! I have Wednesday at 9:00 AM or Thursday at 10:30 AM open. Which works best?", time: 'Draft' },
    ],
  ...
  },
  {
    ...
    thread: [
      { kind: 'customer', text: 'My gutters are overflowing when it rains. Looking for a cleaning and check on the downspouts.', time: '9:58 AM' },
      { kind: 'auto', text: "Thanks for contacting us! We'll reply within 24 hours.", time: '9:58 AM' },
...
export const LEAD_VOLUME: MockChartPoint[] = [
...
export const PLATFORM_GROWTH: MockChartPoint[] = [
...
export const SERVICES = ['Roof Inspection', 'Roof Repair', 'Gutter Cleaning', 'Window Cleaning', 'Pressure Washing', 'General Inquiry']
export const INDUSTRIES = ['Home Services', 'Landscaping', 'Real Estate', 'Cleaning Services', 'Health & Wellness', 'Roofing', 'Fitness', 'Plumbing', 'Retail', 'Pet Services']

export const FORM_URL = 'https://leadflowpro.com/f/acme-home-services'
